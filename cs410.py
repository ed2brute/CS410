...
# 
# Extracts diagnosis, tissue committe, and SNOMED codes from scanned Ameripath 
# reports received via RightFax by converting the PDF image to text using OCR.
# Indexes the scraped text and provides search capabilities.
# 
#
...
def ReadGYNCytoReports(pages, reports, filepath):
    ...
    for pg_num, page in enumerate(pages, start=1):
        ...
        if results_index > 0 and molecular_index > 0:
            ...
            # Ameripath is bad about having duplicate reports in the fax
...
def ReadSurgReports(pages, reports, filepath):
    # Ameripath reports can mix non-gyn cytology with surgical pathology
...
def SendToFile(reports):
    print(r"Writing to logfile: C:\Users\VHACMOCoberj\Desktop\Ameripath\log_" + str(datetime.now().date()) + ".txt")
    
    f = open(r"C:\Users\VHACMOCoberj\Desktop\Ameripath\log_" + str(datetime.now().date()) + ".txt", "a")
...
# set the path to the image and text processing engines and the ameripath PDF files
...
ameripath_dir = r"C:/Users/VHACMOCoberJ/Desktop/Ameripath"

# read the Ameripath directory
dir_list = os.listdir(ameripath_dir)
...
# open the files in the Ameripath directory and see what type of reports we have
for ameripath_file in dir_list:
    try:
        pdfpages = convert_from_path(ameripath_dir + "/" + ameripath_file, poppler_path=poppler_engine_path)
        ...
        if page_text.find("CYTOLOGY") > 0:
            print("GYN Cytology - " + ameripath_file)
            ReadGYNCytoReports(pdfpages, reports, ameripath_dir + "/" + ameripath_file)        
        elif page_text.find("Indianapolis,") > 0:
            print("Dermatopathology - " + ameripath_file)
            ReadDermReports(pdfpages, reports, ameripath_dir + "/" + ameripath_file)        
        elif page_text.find("Denver,") > 0 and page_text.find("AmeriPath") > 0:
            print("Surgical pathology, non-GYN cytology, and supplimental reports - " + ameripath_file)
            ReadSurgReports(pdfpages, reports, ameripath_dir + "/" + ameripath_file)        
        else:
            print("Non Anatomic Pathology file encountered: " + ameripath_file)
    except:
        print("Skipping  '" + ameripath_file + "' -- not a PDF or other error")
#end for
...
ix = index.create_in(ameripath_dir, schema)
writer = ix.writer()
